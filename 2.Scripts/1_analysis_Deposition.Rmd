...
library(ggplot2);library(lme4);library(DHARMa);library(MuMIn); library(glmmTMB);library(knitr);library(bipartite);library(iNEXT);library(vegan);library(igraph);library(cowplot);library(stringr)
...
ggplot(emS)+ 
  ...
  scale_colour_manual(values=c("#e72881", "#63077D","#1C796F","#FF6A00"), guide="none")+
...
```
We observe that the four functional groups vary in their hability to deposit pollen on the stigmas, been the anther buzz bees the most efficient. Flower buzz also differ from the other two groups (Robber and Theft). We also observe variation within the bee species belonging to the same functional group, as shown below:
```{r Variation in Pollen Deposition among Bee Species}
...
ggplot(efic, aes(x=bee_sp, y=polen_depo, fill=func_group)) +
  ...
  scale_fill_manual(values=c("#e72881", "#63077D","#1C796F","#FF6A00"), guide="none")+
...
rede_deposicao = read.csv2("1.RawData/rede_deposicao.csv", header = TRUE, row.names = 1) #deposition metric = mean pollen deposited per interaction into the stigma
pollinator_groups <- tapply(efic$func_group, efic$bee_sp, function(x) unique(x)[1])

table(efic$func_group, efic$bee_sp)
pollinators_net <- colnames(rede_deposicao)

setdiff(pollinators_net, names(pollinator_groups))#some names are not matching between the data.frames
setdiff(names(pollinator_groups), pollinators_net)

pollinators_net_corrected <- gsub("0", "na", pollinators_net)
colnames(rede_deposicao) <- pollinators_net_corrected

setdiff(pollinators_net_corrected, names(pollinator_groups))
setdiff(names(pollinator_groups), pollinators_net_corrected)
...
pollinator_groups_ordered <- pollinator_groups[colnames(rede_deposicao)]

pollinator_colors <- group_colors[pollinator_groups_ordered]

pollinator_colors[is.na(pollinator_colors)] <- "grey50"

plotweb(rede_deposicao,
        text.rot = 90,
        col.low = "#999999",            # plants
        col.high = pollinator_colors,   # polinators
        bor.col.low = "#000000",
        bor.col.high = "black",
        bor.col.interaction = "#333333",
        col.interaction = "#666666")

```

And we can combine the frequency of visits with the total amount of pollen grains to explore the pollinators role in been efficient in pollen deposition.
...
pollinators_net <- colnames(rede_eficacia_f)
pollinator_groups_ordered <- pollinator_groups[colnames(rede_eficacia_f)]
